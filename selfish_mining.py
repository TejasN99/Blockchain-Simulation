...
def write_logs_for_nodes():
    ...
    2) overall_stats.txt
    """
    log_folder = "Log_I_" + str(I) + "_ttx_" + str(ttx) + "_nodes_" + str(total_nodes) + "_z0_" + str(z0) + "_zeta_" + str(zeta) + "_TXNS_" + str(TOTAL_TXNS) +"alpha"+ str(alpha)
    ...
    f.write("Total number of blocks in blockchain: " + str(len(node.seen_block_id)) + "\n")
...
def event_handler(event_list, event, ttx):
    ...
    # Handing a generate_block event would indicate that a node has completed its mining
        ...
        #Shashank: This update only happens for Honest Nodes. 
        # For Adversary nodes, leaf blocks do not change (Official BlockChain remains as it is) 
        if not isinstance(src_node,Adversary) or (isinstance(src_node,Adversary) and src_node.public_release==1):
            src_node.leaf_blocks.remove(src_node.mining_on)
            src_node.leaf_blocks.append(new_block)
        
        #Shashank: Mining on will change for both honest and adversary
        ...
        #Case:Honest Node has mined or Adversary has publicly released a mined block
        if not isinstance(src_node,Adversary) or (isinstance(src_node,Adversary) and src_node.public_release==1):
        ...
        #Switch off Public Release Flag for Adversary Nodes
        if isinstance(src_node,Adversary) and src_node.public_release==1:
            src_node.public_release=0
            ...
            if not isinstance(src_node,Adversary) or (isinstance(src_node,Adversary) and src_node.public_release==1):
                ...
                if (src_node.mining_on.chain_length-new_block.chain_length <src_node.lead):
                    if (src_node.lead>2):
                        ...
                        src_node.pvt_chain=src_node.pvt_chain[1:]
                        src_node.public_release=1
                        print("Node", src_node.id,"publicly releases a private block as Honest reduced lead by 1")
                        c+=1 
                        new_mining_event = Event(event.execution_time +c, "generate_block", pvt_block, src_node)
                        print("Event Count",event_count,"publicly releases a private block as Honest reduced lead by 1",src_node.id)
                        hq.heappush(event_list,(event.execution_time +c, new_mining_event))
                    
                    elif (src_node.lead==2):
                        src_node.lead=0
                        pvt_block1=src_node.pvt_chain[0]
                        pvt_block2=src_node.pvt_chain[1]
                        src_node.pvt_chain=src_node.pvt_chain[2:]
                        src_node.public_release=1
                        print("Node", src_node.id,"publicly releases 2 private blocks as Honest reduced lead by 1")
                        
                        #Release Pvt Block 1
                        c+=1 
                        new_mining_event = Event(event.execution_time +c, "generate_block", pvt_block1, src_node)
                        print("Event Count",event_count,"publicly releases first private block as Honest reduced lead by 1",src_node.id)
                        hq.heappush(event_list,(event.execution_time +c, new_mining_event))

                        #Release Pvt Block 2
                        c+=1
                        event_count+=1
                        new_mining_event = Event(event.execution_time +c, "generate_block", pvt_block2, src_node)
                        print("Event Count",event_count,"publicly releases second private block as Honest reduced lead by 1",src_node.id)
                        hq.heappush(event_list,(event.execution_time +c, new_mining_event))

                    elif (src_node.lead==1):
                        src_node.lead='0_'
                        pvt_block=src_node.pvt_chain[0]
                        src_node.pvt_chain=src_node.pvt_chain[1:]
                        src_node.public_release=1
                        print("Node", src_node.id,"publicly releases a private block as Lead was 1")
                        c+=1 
                        new_mining_event = Event(event.execution_time +c, "generate_block", pvt_block, src_node)
                        print("Event Count",event_count,"publicly releases a private block as Lead was 1",src_node.id)
                        hq.heappush(event_list,(event.execution_time +c, new_mining_event))

                    elif (src_node.lead=='0_' or src_node.lead==0):
                        src_node.lead= 0
                        #new_mining_event = Event(event.execution_time +c, "generate_block", new_block, src_node)
...
nodes[total_nodes + 1] = Adversary(i+1, 'Fast',  genesis_block, I/alpha)
