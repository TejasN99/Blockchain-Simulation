...
def event_handler(event_list, event, ttx):
    ...
    # Handing a generate_block event would indicate that a node has completed its mining
        ...
        #Shashank: This update only happens for Honest Nodes or a Public Release by Adversary Node 
        # For Adversary nodes which have privately mined a block, leaf blocks do not change (Official BlockChain remains as it is) 
        ...
        #We dont trigger receive block to peers
            ...
            if (src_node.lead!='0_'):
                print("Lead =",src_node.lead,"case")
                print("Len of pvt chain b4 append",len(src_node.pvt_chain))
                src_node.lead+=1
                src_node.pvt_chain.append(new_block)
            else:
                print("0_ case")
                print("Ideally Adversary_release_status should be public",new_block.adversary_release_status)
                print("Len of pvt chain",len(src_node.pvt_chain))
                src_node.lead=0
    ...
    # Handing a receive_txn event would indicate that a node has received a new block. It spawns more receive_block events so as to forward the block
        ...
        if isinstance(src_node,Adversary):
            print("Inside generate_block of Adversary Node",src_node.id)
            print("Lead",src_node.lead)
            print("Adversary_release_status",new_block.adversary_release_status)
            print("Pvt Chain length",len(src_node.pvt_chain))
        ...
        if new_block.id not in src_node.seen_block_id:
            ...
            if not isinstance(src_node,Adversary):
                ...
                if isinstance(src_node.lead,int):
                    LEAD=src_node.lead
                else: # 0_ case
                    LEAD=0    
                if (src_node.mining_on.chain_length-new_block.chain_length <LEAD):
                    
                    if (src_node.lead=='0_' or src_node.lead==0):
                        print("Lead= 0_ or 0 will move to 0")
                        src_node.lead= 0
                    
                    elif (src_node.lead>2):
                        print("Lead is >2 and =",src_node.lead)
                        src_node.lead-=1
                        ...
                        print("Lead=2 will move to 0")
                        src_node.lead=0
                        pvt_block1=src_node.pvt_chain[0]
                        pvt_block2=src_node.pvt_chain[1]
                        pvt_block1.adversary_release_status='public'
                        pvt_block2.adversary_release_status='public'
                        src_node.pvt_chain=[]
                        
                        print("Node", src_node.id,"publicly releases 2 private blocks as Honest reduced lead by 2")
                        
                        #Release Pvt Block 1
                        c+=1 
                        new_mining_event = Event(event.execution_time +c, "generate_block", pvt_block1, src_node)
                        print("Event Count",event_count,"publicly releases first private block", pvt_block1.id,"as Honest reduced lead by 1",src_node.id)
                        hq.heappush(event_list,(event.execution_time +c, new_mining_event))
                        ...
                        print("Event Count",event_count,"publicly releases second private block", pvt_block2.id, "as Honest reduced lead by 1",src_node.id)
                        hq.heappush(event_list,(event.execution_time +c, new_mining_event))

                    elif (src_node.lead==1):
                        print("Lead=1 will move to 0_")
                        src_node.lead='0_'
                        pvt_block=src_node.pvt_chain[0]
                        src_node.pvt_chain=[]
                        pvt_block.adversary_release_status='public'
                        print("Node", src_node.id,"publicly releases a private block",pvt_block.id,"as Lead was 1")
                        c+=1 
                        new_mining_event = Event(event.execution_time +c, "generate_block", pvt_block, src_node)
                        print("Event Count",event_count,"publicly releases a private block",pvt_block.id," as Lead was 1",src_node.id)
                        hq.heappush(event_list,(event.execution_time +c, new_mining_event))                
