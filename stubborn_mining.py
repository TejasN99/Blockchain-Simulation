...
def write_logs_for_nodes():
    ...
    adversary_blocks_private_ids = [block.id for block in adversary_blocks_private]
    adversary_blocks_public_ids = [block.id for block in adversary_blocks_public]

    print("Adversary blocks private:", str(adversary_blocks_private_ids))
    print("Adversary blocks public:", str(adversary_blocks_public_ids))
    ...
    f.write("Adversary blocks private:" + str(adversary_blocks_private_ids)+ "\n")
    f.write("Adversary blocks public:" + str(adversary_blocks_public_ids)+ "\n")
    ...
    adv_blocks_in_chain=[]
    for block_id in adversary_blocks_public_ids:
...
def generate_valid_txn_list(node):
    ...
    if len(node.unspent_txn_pool) < MIN_TXNS_BLOCK:        
        ...
        if len(node.seen_txn_id) >= TOTAL_TXNS:
            event_list = []
            adversary_node = nodes[total_nodes + 1]
            c = 0
            for block in list(set(adversary_blocks_private) - set(adversary_blocks_public)):
                block.adversary_release_status = "public"
                print("Node", adversary_node.id,"publicly releases a private block",block.id, "at end of simulation")        
                new_mining_event = Event(c, "generate_block", block, adversary_node)
                hq.heappush(event_list,(c, new_mining_event))
                c += 1
    ...
    # Handing a generate_block event would indicate that a node has completed its mining
        ...
        if isinstance(src_node,Adversary):
            if (new_block.adversary_release_status=='public'):
                adversary_blocks_public.append(new_block)
            else:
                adversary_blocks_private.append(new_block)
